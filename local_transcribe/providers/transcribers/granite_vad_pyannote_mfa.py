#!/usr/bin/env python3
"""
Combined Transcriber+Aligner plugin using IBM Granite with VAD-based segmentation and MFA alignment.

This plugin combines:
- Pyannote VAD for intelligent audio segmentation at speech boundaries
- Granite's transcription capabilities
- Montreal Forced Aligner for precise word-level timestamps

Unlike the fixed-time chunking in granite_mfa, this plugin uses VAD-informed "Cut & Merge"
segmentation that respects natural speech boundaries, reducing word truncation at chunk
boundaries and improving both transcription accuracy and timestamp precision.

The integrated stitcher produces continuous WordSegments output, requiring no downstream changes.
"""
...
from local_transcribe.lib.vad_pyannote_segmenter import VADSegmenter


class GraniteVADMFATranscriberProvider(TranscriberProvider):
    """Combined transcriber+aligner using IBM Granite + VAD segmentation + MFA alignment."""

    def __init__(self):
        self.logger = get_logger()
        self.logger.info("Initializing Granite VAD MFA Transcriber Provider")
        ...
        # VAD configuration (45s max as specified)
    ...
    def get_required_models(self, selected_model: Optional[str] = None) -> List[str]:
        """Return required Granite models (MFA and pyannote models handled separately)."""
    ...
    def _init_vad_segmenter(self):
        """Initialize the VAD segmenter if not already done."""
    ...
    def transcribe(self, audio_path: str, device: Optional[str] = None, **kwargs):
        ...
        raise NotImplementedError(
            "granite_vad_mfa is a combined transcriber+aligner. Use transcribe_with_alignment() instead."
        )
    ...
    def transcribe_with_alignment(
        ...
        **kwargs
        ...
        """
        Transcribe audio with VAD-based segmentation and MFA alignment.
        ...
        if debug_enabled and intermediate_dir:
            ...
            debug_dir = pathlib.Path(intermediate_dir) / "transcription_alignment" / "granite_vad_mfa_debug" / timestamp
        ...
        log_progress("Running VAD segmentation...")
        vad_segments = self.vad_segmenter.segment_audio(wav, sr)
        
        log_progress(f"VAD produced {len(vad_segments)} segments")
        ...
        if verbose:
            log_completion(f"Transcription complete: {len(result)} words from {len(vad_segments)} VAD segments")
...
def register_transcriber_plugins():
    ...
    registry.register_transcriber_provider(GraniteVADMFATranscriberProvider())

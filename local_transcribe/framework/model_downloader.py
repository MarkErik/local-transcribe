...
def ensure_models_available(providers, models_dir: Path, args) -> int:
    ...
    Args:
        providers: Dict containing 'transcriber', 'aligner', and/or 'diarization' providers
    ...
    is_remote_transcriber = getattr(args, 'transcriber_provider', None) == 'remote'
    if is_remote_transcriber:
        print(f"[*] Remote transcription enabled - skipping local transcriber model download")
    ...
    # Only check transcriber models if provider is present (it won't be if using remote transcriber)

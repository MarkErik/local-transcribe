...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--transcript-cleanup-url", help="URL for remote transcript cleanup provider (e.g., http://ip:port for Llama.cpp server) [Default: http://localhost:8080]")
    ...
    p.add_argument("--llm-stitcher-url", help="URL for LLM stitcher provider (e.g., http://ip:port for LLM server) [Default: http://0.0.0.0:8080]")
    p.add_argument("--llm-turn-builder-url", help="URL for LLM turn builder provider (e.g., http://ip:port for LLM server) [Default: http://0.0.0.0:8080]")
        ...
        if args.transcriber_provider == "granite":
            ...
            while True:
                ...
                if choice in ['1', '']:
                    ...
                    print(f"✓ Selected: LLM stitching with URL: {llm_url}")
                    break
                else:
                    print("Error: Please enter 1 or 2.")

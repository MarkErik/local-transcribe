...
def compare_chunks(words1, words2, llm_url, chunk_size=400):
    ...
    for idx, (chunk1, chunk2) in enumerate(chunks):
        ...
        try:
            response = requests.post(llm_url, json={"model": "local", "prompt": prompt, "max_tokens": 2048})
            response.raise_for_status()
            result = response.json()
            summary = result.get("choices", [{}])[0].get("text", "No response")
...
def main():
    ...
    parser.add_argument("llm_url", help="URL of the LLM endpoint (without http:// or /v1/completions)")
    args = parser.parse_args()
    
    llm_url = args.llm_url
    if not llm_url.startswith('http://'):
        llm_url = 'http://' + llm_url
    if not llm_url.endswith('/v1/completions'):
        llm_url += '/v1/completions'

    words1 = read_csv_words(args.file1)
    words2 = read_csv_words(args.file2)
    
    compare_chunks(words1, words2, llm_url)

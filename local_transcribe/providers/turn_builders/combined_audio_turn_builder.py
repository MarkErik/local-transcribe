#!/usr/bin/env python3
"""
Combined audio turn builder provider.

This turn builder is used for single audio files with diarization,
...
class CombinedAudioTurnBuilderProvider(TurnBuilderProvider):
    """
    Turn builder for combined audio mode (single audio with diarization).
    
    This builder groups words into turns based on speaker and timing,
    producing a TranscriptFlow with hierarchical structure. In combined
    audio mode, interjection detection is simpler since all audio comes
    from a single source with accurate relative timing.
    """
    ...
    def description(self) -> str:
        return "Multi-speaker turn builder for combined audio with diarization"
    ...
    def build_turns(
        ...
        **kwargs
        ...
        """
        Build turns from words with speaker assignments.
        ...
        self._classify_segments_simple(segments)
    ...
    def _classify_segments_simple(self, segments: List[RawSegment]) -> None:
        """
        ...
        for i, segment in enumerate(segments):
            ...
            # Short segments with acknowledgment patterns
                ...
                if any(p in text_lower for p in self.config.acknowledgment_patterns):
                    segment.is_interjection = True
